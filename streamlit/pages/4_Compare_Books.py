...
def search_books_with_checkouts(_connection, book_name, query_limit=10):
    ...
    from (
        select bib_number, title from books
            where lower(books.title) like lower('%{book_name}%')
               or bib_number like '%{book_name}%'
    ) as searched_books
    join (select bib_number, sum(checkouts) as total_checkouts from checkouts
        group by bib_number
        order by total_checkouts desc) as popular_books
        on popular_books.bib_number = searched_books.bib_number
...
def get_books_ratings(_connection, books_list):
    query = f"""
        ...
        from
        (select reviews.bib_number, substr(reviews.date, 0, 7) as month, avg(reviews.rating) as avg_rating, count(reviews.rating) as count_rating
        from reviews
        where reviews.bib_number in ({books_list})
        group by reviews.bib_number, month) as books_avg
...
def get_books_checkouts(_connection, books_list):
    ...
    select books.bib_number, books.title, checkouts.month, checkouts.checkouts
        from books
        join checkouts
        on books.bib_number = checkouts.bib_number
...
if 'selected_books' not in st.session_state:
    st.session_state.selected_books = pd.DataFrame()
...
with select:
    ...
    search_book = st.text_input("Book Title or Bib Number",
        placeholder='Book Title or Bib Number',
    )

    df = search_books_with_checkouts(session, search_book)
    ...
    st.header("Selected books")
    selected_books = books.selection.rows
    st.session_state.selected_books = pd.concat([st.session_state.selected_books, df.iloc[selected_books].copy()]).drop_duplicates()
    ...
    if st.button("Clear"):
        books.selection.rows = None
        st.session_state.selected_books = pd.DataFrame()
        st.rerun()

with works:
    books_list = to_list(st.session_state.selected_books.BIB_NUMBER)
    if(len(books_list) < 1):
        st.warning('You must select at least one book')
        st.stop()
    # st.subheader("Books publication year and average review")
    ...
    st.subheader("Other Books By Author(s)")
    authors_list = to_list(get_book_authors_list(session, books_list).AUTHOR_ID)
    ...
    st.subheader("Books Monthly Ratings")
    books_ratings_df = get_books_ratings(session, books_list)
    st.dataframe(
        books_ratings_df,
        column_config=column_configuration,
        use_container_width=True,
    ...
    books_checkouts_df = get_books_checkouts(session, books_list)
    st.dataframe(
        books_checkouts_df,
        column_config=column_configuration,
        use_container_width=True,
...
with compare:
    ...
    min_rating_month = books_ratings_df.MONTH.min()
    max_rating_month = books_ratings_df.MONTH.max()
    ...
    st.subheader("Books Monthly Checkouts")
    min_checkouts_month = books_checkouts_df.MONTH.min()
    max_checkouts_month = books_checkouts_df.MONTH.max()
    ...
    checkouts_series_df = books_checkouts_df[(books_checkouts_df.MONTH >= from_date_c) & (books_checkouts_df.MONTH <= to_date_c)].groupby(by=['TITLE', 'MONTH']).sum().reset_index()
    # checkouts_series_df = pd.pivot_table(authors_checkoutss_df, values='AVG_checkouts', aggfunc='mean', index=['NAME', 'MONTH'] ).reset_index()
    st.line_chart(
        checkouts_series_df.pivot(index='MONTH', columns='TITLE', values='CHECKOUTS').sort_values(by='MONTH')
    )
